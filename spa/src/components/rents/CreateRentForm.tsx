...
import type {Client, Facility, RentForm} from '../../utils/typedefs.ts';
...
function CreateRentForm() {
    ...
    function updateDataToSend(startDateString: string, endDateString: string) {
        const data: RentForm = {
...
//toDo: pytanie do dr. chomatka: czemu jak ustawilem ciemne tło to sie naprawiło jego mać?
    ...
    return(
        <div className="w-3/4 ">
            <form onSubmit={null} id="rent-form" className ="flex flex-col justify-center items-center m-4">
                <label htmlFor="start-date-input" className="m-4">Podaj datę początkową:</label>
                <input type="datetime-local" name="start-date" id="start-date-input "  className="w-full m-4"/>
                <label htmlFor="end-date-input" className="m-4">Podaj datę początkową (opcjonalnie):</label>
                <input type="datetime-local" name="end-date" id="end-date-input" className="w-full m-4"/>
